	...
	request(url, function (error, response, html) {
		if (!error) {
			...
			$('#GridViewRTI').filter(function() {
				var data = $(this);
				for(i=1;i<10;i++) {
					nextBus = data.find('tr:nth-child('+i+') td:nth-child(1)').text();
					json.nextBus.busNo.push(nextBus);
				}
			...
			$('#GridViewRTI').filter(function() {
				var data = $(this);
				for(i=1;i<10;i++) {
					nextBus = data.find('tr:nth-child('+i+') td:nth-child(2)').text();
			...
			$('#GridViewRTI').filter(function() {
				var data = $(this);
				for(i=1;i<10;i++) {
					nextBus = data.find('tr:nth-child('+i+') td:nth-child(3)').text();
